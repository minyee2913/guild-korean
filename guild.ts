...
import { open, readFile, readFileSync, writeFileSync, existsSync, mkdirSync } from "fs";
...
let ex = existsSync("../2913plugins/guild");
if (ex == false) {
    mkdirSync("../2913plugins/guild");
...
function addXp(xp:number, playerName:string){
    if (dataJs.find((e:any, i:any) => e.Name == playerName) != undefined) {
        let data = dataJs.find((e:any) => e.Name == playerName);
        let id = data.guildID;
        let IdJs = guildJs.find((e:any) => e.guildID == data.guildID);
